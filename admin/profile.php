...
$s = $_SESSION['username'];
$select = $con->prepare("SELECT * FROM soccer_admin WHERE username = '$s' ");
...
$u = $row['username'];
$p = $row['password'];
$update = $con->prepare("UPDATE soccer_admin SET username = '$uname', email = '$email', phone = '$phone' , password = '$p' WHERE id = '$id' ");
if ($update->execute()) {
  echo "<i style='color:green;'>Your profile is updated successfully</i>";
  
  $select = $con->prepare("SELECT * FROM soccer_admin WHERE id = '$id' ");
$select->setFetchMode(PDO::FETCH_ASSOC);
$select->execute();
$r = $select->fetch();
$ur = $r['username']; 
...
$u = $_SESSION['username'];
$view = $con->prepare("SELECT * FROM soccer_admin WHERE username = '$u' LIMIT 1");
...
$view->execute();
 while ($data = $view->fetch()) { 
    ...
    $_SESSION['admin'] = $data['identifier'];
